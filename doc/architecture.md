...
| kill_enemy -> take_cover | 7 | yes; leaving fire | VETO; InCover churn while seeing |
| kill_enemy -> crit_wounded | 5 | wounded | never veto |

Three rules explain every row:

1. **Veto only the exits from a fire action.** `kill_enemy` and `kill_if_not_visible` fire on `visible_now` regardless of `InCover`, so holding one keeps the NPC shooting. The watched exits are the two that leave `kill_enemy` toward cover while the enemy is still visible, from its preconditions: `-> take_cover` (`InCover` dropped on cover-manager churn) and `-> get_ready` (`ReadyToKill` dropped on reload/empty/misfire). Adding `-> get_ready` (the dominant first hop) is the gain over the old take_cover-only rule; holding it reloads the NPC in place (the object handler reloads independent of the combat action) instead of the `get_ready -> take_cover -> kill_enemy` detour. `-> look_out` is NOT watched: it requires `SeeEnemy = false`, which the `sees` gate always declines, so it could never hold.
2. **Never veto `get_ready -> take_cover`, the fat 118-count edge.** It is the tempting target (it is where the shuffle visibly enters cover), but `get_ready_to_kill` set `InCover = false` and `take_cover` is the ONLY action that restores it; blocking that edge strands a non-firing NPC that can never satisfy `kill_enemy`'s `InCover` precondition. The shuffle is caught one hop upstream instead, at `kill_enemy -> get_ready`, so `get_ready` is never entered and this edge is never proposed.
3. **The rest passes for free.** The lost-sight cycle (`take_cover -> look_out`, `look_out -> hold_position`, and the `-> get_ready` edges out of non-fire states) runs with `SeeEnemy = false`; even inside the watched set the advantage gate (`sees`) would decline it, so it needs no special case. Survival and opportunity transitions (`-> crit_wounded`, grenade dodge, `-> retreat`, `-> get_distance`) must always pass and are excluded by the `to_op` whitelist. `sudden_attack -> take_cover` is deferred: low volume and ambush semantics not yet analysed.
...
MCM page: Effectiveness > Reaction (`at_reaction.script`, built 2026-07-11). Per-NPC, rank-tiered aim and vision through the merged #594 engine binds, set once per `npc_on_net_spawn` (the fields are not serialized; the game object is reconstructed on re-online, so every spawn re-sets them). The spawn body is public as `at_reaction.apply(npc)` because the fields are never re-read: a runtime rank change (`npc:set_character_rank`, the bench harness) leaves the old tier applied until a caller re-keys it. Three values per tier:
...
- **Target lead** (predict_time, seconds): the aim point is the target's visible position plus its horizontal velocity x predict_time (`predict_object_position`, `sight_action.cpp:375-384`). 0.40 vanilla to 0.28 hardcore per rank; both compensate tracking latency (far above ballistic need), so low ranks aim further ahead and miss wider on direction changes.
