...
| Reaction | `at_reaction.script` | Effectiveness > Reaction | per-curve `aim_enabled` / `lead_enabled` / `discipline_enabled`, no master (vision moved to Perception > Vision) | All NPCs | callback |
...
- **net_spawn** (`npc_on_net_spawn`): VISION_RANGE via `xcombat.set_view_distance_factor` (n038, no-op on a floor exe), and the AURA via `start_particles`. resolve returns nil for a former carrier, so the bind is written to the neutral value and a dropped optic never keeps a stale factor; the aura is reconciled (started for a carrier, stopped otherwise) and re-attached each online because the particle dies with the game object.
...
| Reaction | Per-NPC aim (min_speed + predict), vision speed, fire-queue scales at net_spawn; the rank VISION_RANGE slice registered as a provider | `xcombat.set_aim_params`, `set_vision_speed`, `set_fire_queue_scale`; `at_effects_resolver.register` |
