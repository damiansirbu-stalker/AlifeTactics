...
- `move` — the moving-fire curve, applied only while the shooter's `move_type` is walk/run (`ai_stalker_fire.cpp:81-104`). Cancels part of the movement spread penalty per rank: novice 1.00 keeps the full vanilla penalty, legend 0.50 shoots on the move as steadily as standing — the stance table's own standing-to-walking ratio. NPCs fire while moving almost exclusively, so this is where rank shows. Error budget: `doc/library/modding/npc-combat-effectiveness.md` "The error budget per shot".
