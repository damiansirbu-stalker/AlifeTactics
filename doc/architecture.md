...
Each curve has an MCM toggle - `aim_enabled` (tracking speed and lock together), `vision_enabled`, `lead_enabled`, `discipline_enabled` - and its own per-rank slider set (`rct_aim_*`, `rct_track_*`, `rct_vision_*`, `rct_lead_*`, `rct_qsize_*`/`rct_qinterval_*`); there is no page master. `apply()` sets tracking speed and lock behind `aim_enabled` and vision behind `vision_enabled`, `_on_shot_lead` runs behind `lead_enabled`. Each subsystem's explanation is the hover hint on its toggle, not a separate row.
