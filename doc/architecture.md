...
Each row opens on its situation: **too_close** (enemy inside the weapon's minimum range) for kite, **grenade** (a grenade danger within ~3s) or **hurt** (health below `hurt_frac`) for flee and retreat, **stalled** (held position ~4s while the enemy is not closing) for snipe. The walk takes the first row whose situation holds, whose palette matches, and whose resolver yields a destination.
