...
| Commitment | `at_commitment.script` | Effectiveness > Commitment | `commitment_enabled` | All NPCs | callback |
...
- **Target lead** (predict_time, seconds): derived from the same tier, not MCM-exposed. 0.40 vanilla to 0.28 hardcore; low ranks overlead a mover.
- **Vision speed** (`xcombat.set_vision_speed`): a factor on `get_visible_value` accumulation (`visual_memory_manager.cpp:365-379`), multiplying the output of whichever detection stack the modpack installed (engine formula or Lua functor), so 1.0 changes nothing anywhere. Mid/long-range only; inside `always_visible_distance` the engine skips the formula.
