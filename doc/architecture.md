...
| `at_gear.script` | feature | done (functional NPC inventory: artefact channels, optics, carrier aura, capped effects) |
...
The MCM menu is a six-category gameplay tree plus a Development tab, the canonical structure (source of truth `at_mcm.script`; the page names here are the MCM labels): **Combat** (Maneuvers, Commitment, Behaviors), **Effectiveness** (Accuracy, Disclosure, Crossfire, Reaction, Range, Resistance), **Danger** (Sound, Hit, Fixes), **Mechanics** (Healing, Jamming, Ammo), **Effects** (wip), **Mutants** (wip), and **Development** (log level, ballistics debug, world-behaviour debug, debug HUD toggle and position, reset to defaults). Danger is its own category because it is perception-and-reaction, not combat skill: its leaves are named by what the NPC perceives - Sound (heard gunfire, the `attack_sound` reaction), Hit (react to distant hits), and Fixes (the always-on crash fixes plus the player-range tuning toggle). The distant-hit reaction (duck) lives here; the return-fire-at-reach answer lives in Effectiveness > Range - same scenario, two features, two homes. Behaviors, Effects, Mutants, and the Range/Resistance pages are wip; Commitment and Reaction are built (`at_commitment.script`, `at_reaction.script`), active on exes carrying the n023/n024/n025 binds and inert with an INACTIVE log line on older ones. One leaf = one name = one `at_<leaf>.script` = one MCM page = one master toggle; the system sections below follow the category order. The Scope column is the scope rule below, at a glance.
...
| Danger | `at_danger.script` | Danger > Sound / Hit / Fixes | fixes always-on; three toggles: Enemy gunfire (Sound), Distant hits (Hit), Player ranges (Fixes) | All NPCs | scheme-patch |
...
| Reaction | `at_reaction.script` | Effectiveness > Reaction | per-curve `aim_enabled` / `vision_enabled` / `lead_enabled` / `discipline_enabled`, no master | All NPCs | callback |
...
The third combat system (`at_stance.script`, MCM Combat > Conduct): small habits applied to VANILLA-driven NPCs at moments the engine already decides — no takeover, no held actions. The engine fires `npc_on_combat_set_body_state` whenever a vanilla combat action sets the body state (the `COMBAT_BODY_STATE_OVERRIDE` forwarder in `callbacks_gameobject.script`; a maneuver-held NPC never reaches it, his planner is blocked and his row sets posture — clean exclusivity for free). The subscriber overrides the posture for long-weapon carriers (`w_rifle`/`w_sniper`) of experienced tier and up, on the stationary ops only (`get_ready_to_kill`/`take_cover`/`look_out`/`hold_position`; overriding a moving detour would slow it for nothing). A per-weapon distance floor keeps it a firing-line behaviour: it crouches only when the enemy is past `CONDUCT_FLOOR_FRAC` (0.3) of the weapon's effective range (`xcombat.WEAPON_RANGES`, rifle 80m / sniper 150m — a rifleman past ~24m, a sniper past ~45m), so a close fight stays standing and mobile instead of crouching at the actor's feet, and the one canonical range table means it never drifts from the maneuver ranges. The decision is HELD, not per-ask: the engine re-asks its combat body state ~8 times a second while an action runs, and the low-cover field toward the enemy jitters 0.0-0.4 second to second, so a stateless per-ask decision flapped crouch/stand continuously (1658 overrides in the first field session — the up-down shuffle, with the T-pose and glide artifacts of posture contention under hit animations). The built form samples `level.low_cover_in_direction` once per `DECIDE_MS` (3s, early re-decide on a combat-op change), resolves it through a hysteresis band (enter crouch at 0.25, leave below 0.15, hold between — the pickoff enter/exit shape; 0.2 center = vanilla's own threshold for this read, axr_npc_vs_heli.script:259), and answers every ask with the held posture, so the applied value never alternates. Both directions override vanilla's blind pick — its crouch behind a random bump (the 1.0.0 `at_stance` bug: it crouched by op alone, never checking the geometry, and NPCs fired into the bump) and its standing tall beside chest-high cover, wasting the largest legitimate accuracy gain the engine has (stillness + crouch, `m_stalker.ltx:476-483`). Traced `[STANCE]` per override, debug-gated. On an exe without the seam the callback never fires and the module is inert.

### The rank curves under a maneuver

A maneuver changes who drives movement and fire intent; it does not exempt the NPC from the skill model. Four of the five rank curves apply at the engine-parameter or per-shot-callback level and reach every NPC shot regardless of what drives it: Accuracy dispersion and Moving Fire (`npc_shot_dispersion`, fired inside the engine's weapon-accuracy calculation on every shot), Tracking Speed and Target Lead (per-NPC engine aim parameters via `set_aim_params`), and Vision Speed (the `get_visible_value` multiplier). A novice under a maneuver still misses, tracks, leads, and spots like a novice.
...
MCM page: Effectiveness > Reaction (`at_reaction.script`, built 2026-07-11). Per-NPC, rank-tiered aim, target lead, vision, and fire discipline through merged engine binds (#594, #603). Aim, vision, and fire discipline are set once per `npc_on_net_spawn` (the fields are not serialized; the game object is reconstructed on re-online, so every spawn re-sets them); the target lead is recomputed live on the fire seam (t156, below). The spawn body is public as `at_reaction.apply(npc)` because the fields are never re-read: a runtime rank change (`npc:set_character_rank`, the bench harness) leaves the old tier applied until a caller re-keys it. Four values per tier:

- **Tracking speed** (`xcombat.set_aim_params` min_speed, rad/s): the sight's fine-tracking cap in `select_speed` (`sight_manager.cpp:80-101`). 0.24 is the vanilla default, 2.5 the hardcore-aim game option's value; the MCM tracking curve spans that dial per rank. The angle pair passes -1 so the live `ai_aim_*` band keeps applying — the design survives the hardcore-aim option and any global cvar tuner. What it does and does not do, tested 2026-07-12 (forced A/B benches, 10x value spread): it does NOT change first-shot latency — mechanically impossible, the fine-track band (`min_angle` 0.196 rad) sits entirely inside the trigger cone (`fire_angle` 0.3927 rad), so the first shot never waits for it — and it did not raise hit% on a strafing target; what it changes is how tightly the barrel visibly follows a mover during fire. The curve ships as exactly that: per-rank tracking tightness, labeled so.
...
- **Vision speed** (`xcombat.set_vision_speed`): a factor on `get_visible_value` accumulation (`visual_memory_manager.cpp:365-379`), applied after whichever detection stack the install runs (engine formula or Lua functor) — a relative rank spread over the installed baseline, the only semantics the seam supports. Lerps 1.00 (novice, baseline untouched) to 2.00 (legend); the endpoint is the ~2x `visibility_threshold` cut widely played modpack tuning gives every NPC. Inert inside `always_visible_distance`.
...
Mechanics: one LAZY per-id cache — the first `at_gear.factor(npc, ch)` read for an unknown id runs one `xinventory.iterate_inventory` pass and stores the resolved set; `npc_on_item_take`/`npc_on_item_drop` re-resolve that id (they fire for pickups, drops, AND NPC-to-NPC `transfer_item` trades — engine chain Inventory.cpp:209/315 -> InventoryOwner callbacks -> xr_motivator.script:194/212) and refresh the spawn-written engine params via the idempotent `at_reaction.apply(npc)`; `server_entity_on_unregister` drops it; `at_gear.refresh(npc)` is the public re-resolve for flows that grant items without a take event (parented spawn). Aura: ONE effect, worn by any artefact carrier whatever the artefact does — the `aura` key in `at_gear.ltx`, currently `anomaly2\burer_prepare`, a colourless space distortion (bone-verified attach at `aura_bone`, stopped at death/unregister/re-resolve, re-attached at `npc_on_net_spawn` — the `ap_ext_object_mutator` lifecycle; particles die only with the game object, no other loss path, per `doc/library/modding/particles.md`). The predicate is one line in `_build_record`: a kept candidate carrying `sect` is artefact-sourced, which optics never are, so binoculars and NVG grant their vision factor without a visual tell. A distorting stalker IS a visible, huntable artefact drop, kevlar and plate inserts included. Only config-referenced particle names are valid in the key: a name living solely inside `particles.xr` can be absent from a modpack shipping its own library and a missing name is an engine fatal with no script-side check (`r4.cpp:738-748`); `anomaly2\burer_prepare` qualifies through `m_burer.ltx:272` `Particle_Gravi_Prepare`. All new logic lives in `at_gear.script`; the four consumer modules each hold a 1-2 line factor read at a write site they already own (the no-pollution rule).
...
| Conduct | Combat body-state override per action application (`csbs_flags.body_state`) | (subscribes to `npc_on_combat_set_body_state`); `level.low_cover_in_direction` |
