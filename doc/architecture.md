...
- `move` — the moving-fire curve, applied only while the shooter's `move_type` is walk/run (`ai_stalker_fire.cpp:81-104`). Cancels part of the movement spread penalty per rank: novice 1.00 keeps the full vanilla ~2x penalty, legend 0.75 sheds half of it — the Fair grade of the reference tunings; a factor of 0.50 would erase the penalty entirely and was rejected as the default. NPCs fire while moving almost exclusively, so this is where rank shows. Error budget: `doc/library/modding/npc-combat-effectiveness.md` "The error budget per shot".
...
MCM: Development > Ballistics (`at_ballistics.script`). An outcome recorder over the engine's shot and impact feeds, reads only, OFF by default with zero cost (no callbacks registered until the toggle). Three independent gates: the toggle drives CAPTURE (in-memory counters), the log level drives OUTPUT (per-bullet lines at DEBUG, minute tables at INFO to `alifetactics_ballistics.log`, a one-line summary mirrored to the main log), and `ballistics_position` drives the on-screen PANEL (rendered by `at_hud`'s machinery, reading the counters through `get_rows`/`get_session` — never the log).
