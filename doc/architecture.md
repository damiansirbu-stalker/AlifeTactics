...
| `at_combat.script` | feature | v2 intermittent takeover: solo maneuver set (counterflank, flee, retreat, kite, snipe), actor-party fights only; squad coordination and enemy openings are the open phases (todo-combat-takeover-v2.md) |
...
Status: built incrementally. On `main`: the GOAP graft (`xcombat.install_takeover`), the maneuver-catalog spine (`at_combat_doctrine`), and the full solo maneuver set — counterflank, flee, retreat, kite, snipe — with the fire discipline owned by `xcombat.set_combat`. It runs only on fights the actor is party to (2026-07-10; the 2026-07-08/09 any-enemy + seize-radius form is retired); squad coordination and enemy openings remain the open phases. Full phase plan + the decision record: `stalker-dev/doc/todo/todo-combat-takeover-v2.md` (t130-t139 + the Plan section). The full-takeover v1 is preserved on the `combat_takeover` branch.
...
_run_monitor (a vanilla time event, every 200ms; per frame the whole monitor costs
  ...
  no maneuver open -> begin check (600ms)
      ...
      catalog walk from the per-NPC cursor, priority: counterflank -> flee -> retreat -> kite -> snipe
...
The burst SHAPE of takeover fire is the second half of that discipline (2026-07-12): the state machinery asks `state_mgr_weapon.get_queue_params` for the burst, and every state without an animation-specific entry falls to the caller's generic `{5,300,0}` override (`state_mgr.script:322`) — full-auto on a pistol, a burst on a sniper rifle. `at_combat.script` monkey-patches `get_queue_params`: for an NPC a maneuver currently drives, the fallback override is swapped for a per-weapon-kind triple sourced from the engine planner's own `[fire_queue_params]` medium band (`m_stalker.ltx:826-909`) — pistol and rifle bursts, single sniper shots. Animation-tuned states keep their own values, the original's `aim_time` machinery runs unchanged, and every NPC not under a maneuver passes through byte-identical. Tunables: `at_combat_config.ltx` `queue_w_*`.
...
The second system, separate from the maneuvers: the anti-shuffle veto (`at_commitment.script`, MCM Combat > Commitment, built 2026-07-11). The engine keystone is merged (the `npc_on_combat_action_switch` veto, demonized n023); the subscriber registers through `xcombat.on_action_switch`, which probes the seam and reports INACTIVE on exes without it. The problem it fixes: vanilla ties sustained fire to being in cover — `kill_enemy` requires `InCover = true` (`stalker_combat_planner.cpp:342-344`) — and the best-cover point churns, so NPCs break contact and shuffle toward sketchy cover even while they are winning the shooting. The shuffle intervention fixes this in place, with no takeover. The deny rule covers the shuffle transitions: leaving an attack action (`kill_enemy`, `kill_if_not_visible`) for `take_cover` while the NPC is at an advantage — he sees his enemy, `xcombat.can_kill_enemy` says his position gives a clear shot, no non-enemy blocks the lane (`can_kill_member`), and the refusal window is under the MCM cap. A recent-hit standdown was tried and removed 2026-07-12: "any hit in the last 3s permits leaving" kept the veto disabled in the one fight it exists for, the one the player is shooting in. Every other transition passes untouched; release needs no machinery because each condition falsifies itself. The debug traces carry the tuning evidence: declines (which condition blocked a would-be veto), the transition histogram (what real shuffling consists of), and the cover churn.
...
- `disp` — the flat rank curve, applied to every shot. `disp = 1.00` preserves the engine's vanilla cone, lower values tighten it. Defaults 1.00 (novice) -> 0.38 (legend).
- `move` — the moving-fire curve, applied ONLY when the shooter's `move_type` is walk/run (the callback receives the same `movement_type()` the engine keys its walk/run stance multipliers on, `ai_stalker_fire.cpp:81-104`). It cancels part of the movement spread penalty per rank: novice 1.00 keeps the full vanilla penalty, legend 0.50 makes his moving fire as steady as his own standing fire — 0.50 is the stance table's own standing-to-walking ratio (`(disp_stand_stand+1)/(disp_walk_stand+1)` across the vanilla rank profiles, ~0.38-0.53), not an invented number. Rationale (bench-measured 2026-07-12): NPCs fire while moving almost exclusively, so the movement penalty is their real accuracy; the cone converts to hits against a static or exposed target, and the moving-fire curve is what lets high ranks keep that pressure while repositioning. Full error budget: `doc/library/modding/npc-combat-effectiveness.md` "The error budget per shot".
...
MCM page: Effectiveness > Reaction (`at_reaction.script`, built 2026-07-11). Per-NPC, rank-tiered aim and vision through the merged #594 engine binds, set once per `npc_on_net_spawn` (the fields are not serialized; the game object is reconstructed on re-online, so every spawn re-sets them). Three values per tier:
...
- **Target lead** (predict_time, seconds): the aim point is the target's visible position plus its horizontal velocity x predict_time (`predict_object_position`, `sight_action.cpp:375-384`), applied while the fire sight action is in its predicted state. 0.40 vanilla to 0.28 hardcore per rank; both values compensate tracking latency, far above ballistic flight time, so low ranks aim further ahead and their aim point is further wrong when the target changes direction.
- **Vision speed** (`xcombat.set_vision_speed`): a factor on `get_visible_value` accumulation (`visual_memory_manager.cpp:365-379`), multiplying the output of whichever detection stack the install runs (engine formula or Lua functor) — a RELATIVE rank spread over the installed baseline, the only semantics the seam supports (an absolute per-rank detection speed is not expressible: modpack stacks replace the whole formula through the Lua functor, so no scalar maps between installs). The curve lerps 1.00 (novice = the installed baseline, untouched) to 2.00 (legend), the endpoint magnitude sourced from widely played modpack tuning that cuts `visibility_threshold` 150->80 (free) / 75->35 (danger) vs vanilla, ~2x detection speed for every NPC. Mid/long-range only; inside `always_visible_distance` the engine skips the formula.
